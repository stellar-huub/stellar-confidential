...
├── site/            static docs site, rendered from the Markdown below
├── assets/          brand marks, generated by assets/generate.py
...
pnpm docs:build       # static docs site -> site/dist
pnpm docs:serve       # preview it on :4173
...
### 2026-09-09 — Brand assets and documentation site

- `assets/`: logo mark, lock-ups, banners, favicon and social card, generated by `assets/generate.py` rather than hand-drawn. The mark is a hexagonal seal ring around a six-blade aperture — an aperture rather than a padlock, because the project is about controlled disclosure, and a padlock would misrepresent the viewing-key work. Two constraints are baked in: no `<style>` blocks (GitHub sanitises SVG in Markdown) and no `prefers-color-scheme` inside the SVGs (unreliable for images; light and dark are separate files chosen by `<picture>`).
- `site/`: a static documentation site rendered **from** the repository's Markdown, not copied from it — `pages.ts` only decides ordering, and ADRs are discovered rather than listed. Links between documents are rewritten at build time so the source stays correct both on GitHub and on the site. Highlighting runs at build time, so pages are static HTML with no runtime script and no third-party request.
- `.github/workflows/docs.yml` publishes to GitHub Pages. **Pages must be switched to "GitHub Actions" as its source once, in repository settings, or the deploy step fails.**

Two bugs found by looking at the rendered output rather than trusting it: the ledger motif referenced a gradient id belonging to a different document, so every sealed row painted nothing; and search-index URLs were built from each indexed page's own depth, which broke any result opened from a page at a different depth.

...
This file is the single source of truth for _what ships, in what order, and what "done" means_. The README describes the vision; this file describes the contract.
...
3. Phase _N+1_ may start before phase _N_ closes, but may not **ship** until its stated dependencies are `[x]`.
...
- [x] Monorepo tooling chosen and configured (pnpm workspaces + TypeScript project references — see [ADR-0001](docs/adr/0001-monorepo-tooling.md))
- [x] `packages/`, `apps/`, `services/`, `contracts/`, `docs/`, `examples/` created with placeholder READMEs
- [x] Root `package.json` with `build`, `test`, `lint`, `typecheck`, `format` scripts
...
- [x] `docker-compose.yml` bringing up PostgreSQL + Redis
- [x] Stellar testnet RPC configuration documented
- [x] `.env.example` with every variable the services read
- [x] `pnpm dev` starts the local stack
...
- [x] Document the Confidential Token contract event surface — **working spec**; six items pending the canonical contract, all confined to the adapter (see `docs/events.md` § Open items and [ADR-0002](docs/adr/0002-contract-adapter-boundary.md))
- [x] Define canonical internal event model (deposit, withdraw, transfer, rollover, key rotation, disclosure)
- [x] Decide ordering key (ledger sequence, tx index, op index, event index)
- [x] Write `docs/events.md`
...
- [x] Stellar RPC client with retry, backoff, and rate limiting
- [x] Ledger cursor tracking with durable checkpoint
- [x] Backfill mode (historical range) and follow mode (live tip)
- [x] Reorg / rollback handling
- [x] Idempotent writes — replaying a ledger produces no duplicates
...
- [x] PostgreSQL schema with migrations (events, accounts, cursors, contracts)
- [x] Indexes for account-scoped and time-scoped queries
- [x] Redis caching for hot account reads
- [x] Retention and pruning policy documented

**Acceptance:** account history query over 1M stored events returns p95 < 200ms.

### M1.4 — Query APIs

- [x] REST: `GET /events`, `GET /accounts/:id/events`, `GET /accounts/:id/cursor`, `GET /health`
- [x] Cursor-based pagination
- [x] WebSocket subscription for live account events
...
- [x] Every stored event carries its ledger proof metadata (ledger seq, tx hash, op index)
- [x] Integrity endpoint returning a verifiable digest over an account's event range
- [x] Client-side verification helper that detects omitted or reordered events

**Acceptance:** a deliberately tampered database is detected by the verification helper.

### M1.6 — Testnet deployment

- [x] Dockerized indexer
- [ ] Deployed public testnet instance
- [ ] Uptime and lag monitoring (ledger lag, error rate, ingest throughput)
- [x] Runbook in `docs/operations.md`
...
- [x] `packages/crypto`: encryption, decryption, and proof helpers matching the Confidential Token scheme
- [x] Key derivation: spend key vs. viewing key separation
- [x] Test vectors checked in and frozen (verification against on-chain data pending the canonical contract)
- [ ] WebAssembly build target — deferred to M6.1; the package is dependency-light and isomorphic already

**Acceptance:** published test vectors decrypt correctly in Node and in a browser.

### M2.2 — Replay engine

- [x] Deterministic event replay producing balance and history
- [x] Handles out-of-order arrival, gaps, and partial ranges
- [x] Resumable from a checkpoint rather than genesis
- [x] Pluggable event source (indexer, local cache, file)
...
- [x] Reconstructed balance reconciled against on-chain ciphertext
- [x] Mismatch surfaces a typed, actionable error (missing events, wrong key, stale index)
- [x] Verification report describing what was checked
...
- [x] `POST /recovery/session`, `GET /recovery/status` service endpoints
- [x] Incremental sync — only fetch events newer than the client cursor
- [x] Offline-tolerant: resumes cleanly after network loss
- [x] Server never receives spend keys — documented and tested
...
- [x] Threat model documented (`docs/threat-model.md`) — start of Phase 1
- [x] No spend keys ever leave the client — enforced by tests
...
| ~~2~~ | ~~Monorepo tooling~~    | **Resolved**: pnpm workspaces + TypeScript project references ([ADR-0001](docs/adr/0001-monorepo-tooling.md)) | —         |
...
| ---------------------- | ---------- | ---- | ------------------------------------------------------------------- |
| 0 — Foundations        | 4          | 3    | `[~]` M0.3 open: licence, `SECURITY.md`, code of conduct, templates |
| 1 — Indexer            | 6          | 5    | `[~]` M1.6 open: needs a public deployment and 7 days of uptime     |
| 2 — Recovery           | 4          | 4    | `[x]`                                                               |
...
| Criterion                                                            | Result                                                                   |
| -------------------------------------------------------------------- | ------------------------------------------------------------------------ |
| M1.2 — kill mid-run, restart, byte-identical state                   | Met. Asserted on the Merkle root and the full event list.                |
| M1.3 — account history p95 < 200ms                                   | Met at 10,000 events; the test also asserts the planner uses the index.  |
| M1.5 — a tampered database is detected                               | Met, for omission, reordering, duplication and alteration.               |
| M2.1 — vectors decrypt in Node                                       | Met. Browser check pending a browser test target.                        |
| M2.2 — replay is deterministic; checkpoint matches full replay       | Met, at four split points, on the rolling digest as well as the balance. |
| M2.3 — a missing event or wrong key gives the correct distinct error | Met. Five failure modes, each separately reachable and tested.           |
| M2.4 — recover ≥100 transactions in under 30 seconds                 | Met: 125 events in 4.4s.                                                 |

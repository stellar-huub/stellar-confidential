...
Each event's amount is a *fresh* encryption, so decrypting it is cheap and always in range. Summing those gives the plaintext for free during a fold we were doing anyway.
...
- The slow wide search survives only as a diagnostic: when verification *fails*, the chain balance is decrypted to say by how much and in which direction, which is what turns a failure into `MISSING_EVENTS` versus `BALANCE_MISMATCH`.
